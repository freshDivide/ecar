	...
	this.request = function(options) {
		var self     = this,
			 ...
			 **/
				...
				//=================mis change=============
				var change_commands = new Array(
					'rm', 'duplicate', 'rename', 'mkdir', 'mkfile', 'upload', 'paste', 'edit','put'
				);
				
				for (x in change_commands) {
					if(cmd === change_commands[x]) {
						if(('#elfinder_state').length > 0) {
							$('#elfinder_state').attr('value',"1");
						}
					}
				}
				//========================================
			...
			var hash   = file.hash,
				...
				dialog = function(text) {
					var editor = $('<textarea class="elfinder-file-edit" rows="20">'+text+'</textarea>')
						...
						opts = {
							title   : file.name,
							width   : self.options.dialogWidth || 800,
							buttons : {},
							close   : function() { 
								$(this).elfinderdialog('destroy'); 
							},
